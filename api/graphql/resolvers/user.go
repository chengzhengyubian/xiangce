...
func (r *mutationResolver) ChangeUserPreferences1(ctx context.Context, language *string) (*models.UserPreferences, error) {
	//配置数据库连接
	database := "photoview"
...
func (r *mutationResolver) ChangeUserPreferencesNew(ctx context.Context, language *string) (*models.UserPreferences, error) {
	db := r.DB(ctx)
	user := auth.UserFromContext(ctx)
	if user == nil {
		return nil, auth.ErrUnauthorized
